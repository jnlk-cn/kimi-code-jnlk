...
- Remove the `kimi server` and `kimi web` commands and the TUI `/web` slash command from this fork. Use Ganymede Code for the desktop workspace and `kimi acp` for IDE integration instead.
...
- DeepSeek V4 Pro and Flash work on the official `openai` provider (`api.deepseek.com`), including thinking toggles, `max` effort, and reasoning round-trips during tool calls.
