...
本 fork 在 upstream **0.23.2** 之上合入社区补丁（当前发行版本 **0.24.0**）。若你使用官方 [MoonshotAI/kimi-code](https://github.com/MoonshotAI/kimi-code)，以下差异可能尚未包含。
...
完整变更列表见 [apps/kimi-code/CHANGELOG.md](apps/kimi-code/CHANGELOG.md)（0.24.0 条目）。若你只需要官方 Kimi 模型与上游发行渠道，请继续使用上游项目。
...
安装指定版本（将标签替换为 [Releases](https://github.com/jnlk-cn/kimi-code-jnlk/releases) 中的实际标签，例如 `v0.24.0-jnlk`）：
