...
- **Known third-party provider**：从本 fork 维护的已验证目录（[GitHub `catalog/api.json`](https://raw.githubusercontent.com/jnlk-cn/kimi-code-jnlk/main/catalog/api.json)，白名单见 `catalog-allowlist.json`；当前含 DeepSeek V4）拉取模型目录，选供应商 → 输入 API 密钥 → 选默认模型。完整上游目录可用 `--url https://models.dev/api.json`
...
::: warning 注意
`/provider` 默认目录只包含本 fork 已验证的官方 DeepSeek API（`api.deepseek.com`）。若改用 `--url` 指向完整 models.dev 或其他 registry，其中的供应商可能指向第三方网关。使用 DeepSeek 官方 API 时，请确认 `base_url` 为 `api.deepseek.com`。`type = "openai"` 时，凭证只从 `config.toml` 读取（`api_key` 或 `[providers.*.env]` 中的 `OPENAI_API_KEY`），不会使用 shell 环境变量 `DEEPSEEK_API_KEY`。
:::
