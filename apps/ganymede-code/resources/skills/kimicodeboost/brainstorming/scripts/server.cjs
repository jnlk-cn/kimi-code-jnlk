...
const KIMICODEBOOST_VERSION = readKimiCodeBoostVersion();
...
function readKimiCodeBoostVersion() {
  ...
  for (const manifest of manifests) {
    try {
      ...
      // Plugin may be packaged without package.json; try the Kimi manifest next.
    }
  }
...
function brandMarkup() {
  const version = escapeHtmlText(KIMICODEBOOST_VERSION);
  const text = 'KimiCodeBoost v' + version;

  return '<div class="brand"><a href="https://github.com/jnlk-cn/KimiCodeBoost"><span class="brand-copy">' + text + '</span></a></div>';
}
