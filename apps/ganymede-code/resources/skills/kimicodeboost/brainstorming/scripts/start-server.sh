...
#
# Options:
#   --project-dir <path>  Store session files under <path>/.kimicodeboost/brainstorm/
...
#                         after the user approves the visual assistant).
#   --foreground          Run server in the current terminal (no backgrounding).
#   --background          Force background mode (overrides auto-foreground on harnesses that reap background processes).
...
if [[ -n "$PROJECT_DIR" ]]; then
  SESSION_DIR="${PROJECT_DIR}/.kimicodeboost/brainstorm/${SESSION_ID}"
  # Persist the bound port and key per project so a restart reuses them and an
  # already-open browser tab reconnects to the same URL with a valid cookie.
  export BRAINSTORM_PORT_FILE="${PROJECT_DIR}/.kimicodeboost/brainstorm/.last-port"
  export BRAINSTORM_TOKEN_FILE="${PROJECT_DIR}/.kimicodeboost/brainstorm/.last-token"
else
