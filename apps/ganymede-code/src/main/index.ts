...
async function initializeServices(): Promise<void> {
  ...
  automations = new AutomationManager(appStore, sessions, workspace, emit, notify);
  await sessions.setHostTools(
    createHostTools({
      store: appStore,
      browser: browsers,
      chrome: chromeBridge,
...
function registerIpc(): void {
  ...
  handle(IPC.gitPullRequests, z.string(), (cwd) => requireWorkspace().pullRequests(cwd));
  handle(
  ...
  handle(
    IPC.skillActivate,
    z.object({ sessionId: z.string(), name: z.string(), args: z.string().optional() }),
    ({ sessionId, name, args }) => requireSessions().activateSkill(sessionId, name, args),
  );
  handle(IPC.pluginCommandsList, z.string(), (sessionId) =>
    requireSessions().listPluginCommands(sessionId),
  );
  handle(
    ...
    ({ sessionId, pluginId, commandName, args }) =>
      requireSessions().activatePluginCommand(sessionId, pluginId, commandName, args),
  );
  handle(IPC.mcpList, z.string(), (id) => requireSessions().listMcp(id));
  handle(
    IPC.mcpReconnect,
  ...
  handle(IPC.inboxRead, z.string(), (id) => requireStore().markInboxRead(id));
  handle(
    IPC.memoriesSearch,
...
const createSessionSchema = z.object({
  ...
  interactionMode: z.enum(['agent', 'plan', 'debug', 'multitask', 'ask']).optional(),
    ...
    if (BrowserWindow.getAllWindows().length === 0) {
      void createWindow(false).then(async () => {
        if (mainWindow !== undefined) {
          browsers = new BrowserManager(mainWindow, emit, () => requireStore().getSettings());
          await requireSessions().setHostTools(
            createHostTools({
              store: requireStore(),
              browser: browsers,
