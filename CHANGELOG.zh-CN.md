...
**kimi-code-jnlk** fork 面向用户的发行说明。英文版见 [CHANGELOG.md](CHANGELOG.md)。英文文档页 `docs/en/release-notes/changelog.md` 由 `node docs/scripts/sync-changelog.mjs` 从英文版同步；中文文档页 `docs/zh/release-notes/changelog.md` 由同一脚本从本文件同步。

发版或 bump 版本时，请**同时**维护 [CHANGELOG.md](CHANGELOG.md) 与本文件，保持版本块、分类与条目一一对应。

各产品包变更记录：

- CLI 包：[apps/kimi-code/CHANGELOG.md](apps/kimi-code/CHANGELOG.md)
- 伽利略 Code 桌面端：[apps/ganymede-code/CHANGELOG.md](apps/ganymede-code/CHANGELOG.md)
- VS Code / Cursor 扩展：[apps/kimi-vscode/CHANGELOG.md](apps/kimi-vscode/CHANGELOG.md)

## 0.25.0（2026-07-15）

### 新功能

- 伽利略 Code 排障模式支持完整验证工作流：静默预加载系统性调试技能、注册临时调试点，并在输入框上方展示编号验证步骤与「问题已修复 / 问题未修复」。通过 `/debug` 或模式菜单进入排障，修复后按步骤验证即可。
- 伽利略 Code 工程模式下写入的设计规格（`docs/kimicodeboost/specs/`）会出现在「计划」面板（与实现计划分开展示），写入后自动打开面板，聊天中的规格路径也可点击预览。头脑风暴写完规格后按 ⌘⇧L 打开「计划」即可审阅。
- 伽利略 Code 工程模式在设计审阅通过后引导进入计划模式，书面计划在 Plans 面板经 Build 批准；批准后输入框待办与计划文件 YAML todos 保持同步。完成设计后切换到计划模式，ExitPlanMode 后按 Build 即可。
- 伽利略 Code 新增「工程」交互模式，内建 KimiCodeBoost 工作流技能（头脑风暴、TDD、子代理驱动开发、评审等），并映射到原生计划面板 / 追问 / Todo / 浏览器 / Worktree。通过模式菜单或 `/engineering` 切换。
- 伽利略 Code 新增 Cursor 风格的内联 Questions 条（逐题 Skip/Continue、Other 自由输入），并在回合运行中将后续消息排队；Enter 入队，⌘↵ / Ctrl↵ 立即注入当前回合。
- 伽利略 Code 的计划审批改为 Cursor 风格：在输入框与「计划」面板提供模型选择与分体 Build 按钮。ExitPlanMode 就绪后选择模型，按 Build（⌘↵ / Ctrl↵）即可开始构建。
- 伽利略 Code 运行时数据改用 `GANYMEDE_HOME`（默认 `~/.ganymede`），项目级配置改用 `.ganymede/`，与 Kimi CLI 的 `.kimi-code` 彻底分离；包名改为 `@jnlk.zone/ganymede-code`。若需保留旧配置，请手动复制 `~/.kimi-code` → `~/.ganymede`，以及项目内 `.kimi-code` → `.ganymede`。
- 伽利略 Code 新增右侧「计划」面板，汇总当前项目下所有已创建的计划文件；时间线仅保留一条紧凑提示，撰写计划时隐藏蓝色流式光标，并在侧栏内阅读计划（支持 YAML frontmatter 待办 / 摘要与 Mermaid 图表；外部编辑器为次要入口）。创建计划后从右侧工具坞打开「计划」（⌘⇧L）即可查看。
- 完善伽利略 Code 侧栏「更多功能」：收件箱实时刷新与未读角标、可编辑的已安排任务、本地站点（预览/停止/删除）、拉取请求「让代理修复」、记忆标签与项目范围，以及技能与插件市场与分区管理。打开侧栏更多功能，或运行 `/inbox`、`/scheduled`、`/sites`、`/pulls`、`/memory`、`/skills`。
- 伽利略 Code 新增本地项目索引（路径、全文与本机语义检索），支持输入框 `@codebase`、Agent 代码库搜索工具、`.ganymedeignore`，以及设置页 / 底栏索引进度。打开项目后输入 `@codebase`，或到「设置 → 项目索引」查看与重建。
- 伽利略 Code 的计划模式对齐 Cursor 式审阅体验：时间线 Markdown 计划卡片、专用「开始构建」审批弹窗（批准 / 修订 / 拒绝）、Shift+Tab 循环切换模式、计划模式提示条，以及输入框上方可折叠待办条。通过 `/plan` 或模式菜单进入计划模式；代理调用 ExitPlanMode 后确认开始构建即可。
- 伽利略 Code 审查面板升级为完整 Git 工作流：单文件 diff（含未跟踪文件）、按暂存状态过滤、语法高亮 diff、丢弃改动，以及 fetch/pull/push 与分支切换。在 Git 项目上打开审查面板即可试用。
- 修复伽利略 Code 输入框权限按钮在未创建会话时无效的问题，并将手动 / Auto / YOLO 文案与 CLI 对齐；新增 `/permission`、`/auto`、`/yolo`、`/mode`、`/model`、`/plan`、`/compact`、`/status`、`/usage`。点击权限按钮或输入 `/auto` 即可试用。
- 伽利略 Code 顶栏 IDE 快捷入口改为分体按钮：下拉列出本机扫描到的编辑器，选中后打开项目并记住默认选择。打开项目后使用「已就绪」旁的编辑器控件即可。
- 伽利略 Code 外部编辑器列表扩展到约 30 款（JetBrains 全家桶、Android Studio、Xcode、VSCodium、Trae 等），并使用内置品牌图标。打开项目后使用「已就绪」旁的编辑器分体按钮即可。
- 伽利略 Code 输入框模型标签旁显示上下文用量圆环，点击可查看分类明细（系统提示词、工具定义、规则、技能、子代理定义、会话）。打开会话后点击模型名称左侧圆环即可查看。
- 伽利略 Code 集群模式对齐 CLI：时间线中显示 AgentSwarm 进度网格，支持 `/swarm` 开关与一键发任务、手动权限预提示、集群标记与输入栏角标。运行 `/swarm` 或 `/multitask`；`/agents` 仅列出后台任务（前台子 Agent 在主对话的 AgentSwarm 块中查看）。
- 伽利略 Code 工具块与 Files 面板支持带行号的语法高亮代码视图，HTML 可一键在内置浏览器预览。打开 Files 或 Write 工具结果即可试用。
...
- 在伽利略 Code 任务摘要面板显示 DeepSeek 余额、会话预估费用与高峰单价。使用官方 DeepSeek API 时打开摘要面板即可查看。
- 在伽利略 Code 的审查与拉取请求中，对非 Git 工作区显示清晰空状态，并提供一键「初始化 Git 仓库」。在普通文件夹上打开审查面板即可试用。
- 伽利略 Code 输入框支持拖入文件与粘贴图片作为附件，并加固 + 上下文菜单（添加后聚焦、光标同步联想、任务运行中禁用模式切换）。将文件拖到输入框、粘贴截图，或使用 + 菜单即可试用。
- 伽利略 Code 的「Git同步」侧栏改为只读仪表盘：展示相对远程的领先/落后提交数，以及汇总与已暂存/未暂存行数统计。在 Git 项目上打开「Git同步」面板即可查看。
- 伽利略 Code 在回合结束且编辑过文件后，于输入框上方展示文件编辑摘要（汇总行数、可展开文件列表、审核 / 撤销），并在主会话区悬浮 Codex 风格回合目录；悬停标记时拉长并展示回合预览卡片。完成一次写入或编辑文件的回合即可试用。

### 修复

- 修复伽利略 Code 的启动恢复流程：桌面桥接缺失或渲染器早期报错时稳定显示重新加载界面，并恢复干净的 TypeScript 构建。
- 伽利略 Code 工程模式切换时静默预加载工作流技能，不再立刻占用 Agent 或在时间线展示技能激活消息；澄清与方案选择改为走 AskUserQuestion（Question 条），而非 Markdown A/B/C。通过模式菜单或 `/engineering` 切入工程模式后发送消息即可试用。
- 将关闭的问题提示视为用户未作答，而非隐式选择推荐项。

### 优化

- 伽利略 Code 助理模式输入框对齐 Cursor 体验：按模式切换 placeholder、Shift+Tab 仅在五种核心模式间循环（工程模式仍可通过菜单或 `/engineering` 进入）、⌘./Ctrl+. 打开模式菜单，底部提示显示这些快捷键。聚焦输入框后按 Shift+Tab 或 ⌘./Ctrl+. 即可试用。
- 伽利略 Code 通过按帧合并流式更新、渐进渲染长历史、延迟挂载代码与工具正文、轻量动效和按帧合并面板拖拽，让长会话与连续交互保持流畅。
- 伽利略 Code 主会话以统一的精细排版渲染 GFM 表格、任务列表、图片和代码，并在图表接近视口时渲染 Mermaid。
- 伽利略 Code 时间线中的助手回复与「思考过程」采用流畅渐显，不再显示尾随的蓝色光标。
- 修复伽利略 Code 聊天区自动布局：时间线与输入框共用同一 flex 列，回合目录改为浮层，不再挤压消息列宽度。
- 伽利略 Code 在 Plan 模式下隐藏蓝色流式光标（含尚未创建计划文件时），改为轻量「正在撰写计划…」提示。
- 伽利略 Code 计划审批改为输入框上方内联操作条，可在右侧计划面板审阅正文后再选择开始构建，不再全屏遮挡。
- 引导 Plan 模式撰写 Cursor 风格计划文件：YAML frontmatter（name、overview、todos）与正文中的 Mermaid 图表。
- 伽利略 Code 弹窗与命令面板取消背景虚化，改为实心半透明遮罩。
- 伽利略 Code 顶栏编辑器控件改用内置品牌图标，不再使用缩小后的系统文件图标。
- 伽利略 Code 在索引用户主目录或超大文件夹前弹出确认，支持中途停止索引，并在关闭「项目索引」后立即停止扫描与监听。打开过大目录或到「设置 → 项目索引」即可试用。
- 优化伽利略 Code 的工作区工具坞与桌面交互视觉：统一采用中性选中态，同时保留原有项目侧栏设计，并支持响应式抽屉和更高效的长任务历史浏览。
