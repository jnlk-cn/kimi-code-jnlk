...
describe('compressImageForModel — dimension cap', () => {
  it('scales the longest edge down to MAX_IMAGE_EDGE_PX, preserving aspect', async () => {
    ...
    // 4500x2250 → 3000x1500 (aspect 2:1 preserved; fork default edge 3000).
...
describe('compressImageForModel — byte budget', () => {
  ...
  it('steps down through the 2000px edge before the 1000px fallback', async () => {
    // Fallback-ladder guard, pinned with an explicit 3000px ceiling (this
    // fork's built-in default is also 3000px; the first fallback edge is
    // 2000px): a PNG whose fitted encode is over budget but whose 2000px
    // encode fits must come back at 2000px, not skip straight to 1000px.
...
describe('compressImageForModel — webp', () => {
  it(
    ...
    async () => {
      const source = new Jimp({ width: 3900, height: 1950, color: 0x3366ccff });
      ...
      expect(sniffImageDimensions(result.data)).toEqual({ width: 3000, height: 1500 });
    },
    15_000,
  );
