...
- `apps/ganymede-code` (`@jnlk.zone/ganymede-code`): Ganymede Code / 伽利略 Code Electron desktop client. Embeds `@moonshot-ai/kimi-code-sdk` in the main process (route A); must not import `@moonshot-ai/agent-core`. Runtime data uses `GANYMEDE_HOME` / `.ganymede` (not `.kimi-code`). Browser UI debugging: `pnpm dev:web:ganymede` (see `apps/ganymede-code/README.md`). UI work must follow `apps/ganymede-code/AGENTS.md`; in particular, never use accent-tinted fills, borders, icons, or indicators for the selected state of rounded controls.
...
- When a change is user-facing and will ship in the next fork release, add matching entries to the root [CHANGELOG.md](CHANGELOG.md) and [CHANGELOG.zh-CN.md](CHANGELOG.zh-CN.md) under the current or upcoming version block (Features / Polish / Bug Fixes / Refactors / Other; Chinese sections: 新功能 / 优化 / 修复 / 重构 / 其他). Keep both files in sync, then run `node docs/scripts/sync-changelog.mjs` so the docs-site changelog pages stay aligned.
